<?php
  ...
  $DB_PAGE = "SELECT * FROM penghudata";
    ...
    <script>
        ...
        function initMap(){
            ...
            for (i = 0; i < JS_number.length; i++) {//130275
                Date = JS_Time[i].split(/\s+/);
                sqltime = Date[1].split(':');
                
                if (sqltime[0] == str && Date[2] == str2 ){
                    count = count + 1;
                    var measle = new google.maps.Marker({
                        position: new google.maps.LatLng(JS_latitude[i],JS_longitude[i]),
                        map: map,
                        icon: {
                            url: "https://i.imgur.com/OcYjtgo.png",//https://i.imgur.com/RcsQXFc.png -> redcircle 
